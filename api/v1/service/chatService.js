...
const getActiveChats = async (currentUserId) => {
    ...
    for (const conn of activeConnections) {
        ...
        const partnerId = (conn.senderId && conn.senderId.$id === currentUserId) ?
                           (conn.receiverId ? conn.receiverId.$id : null) :
                           (conn.senderId ? conn.senderId.$id : null);
        ...
        try {
            partnerUser = await appwrite.getDocument(APPWRITE_USERS_COLLECTION_ID, partnerId);
        } catch (err) {
            console.warn(`No user document found for partnerId ${partnerId} in chat: ${err.message}`);
            continue;
        }
        ...
        try {
            ...
            console.warn(`No images found for partnerId ${partnerId} in chat: ${err.message}`);
        }
...
const removeChat = async (currentUserId, connectionId) => {
    ...
    if (!connectionDoc || connectionDoc.status !== 'chat_active' ||
        (connectionDoc.senderId.$id !== currentUserId && connectionDoc.receiverId.$id !== currentUserId)) {
        const error = new Error("Unauthorized or invalid chat connection for removal");
        error.code = 403;
        throw error;
    }
    ...
    if (senderUserDoc) {
        const updatedSenderChatCount = Math.max(0, (senderUserDoc.activeChatCount || 0) - 1);
        await appwrite.updateDocument(
            APPWRITE_USERS_COLLECTION_ID,
            senderUserId,
        ...
        console.warn(`Sender user document not found for ID: ${senderUserId} during chat removal.`);
    }
    ...
    if (receiverUserDoc) {
        const updatedReceiverChatCount = Math.max(0, (receiverUserDoc.activeChatCount || 0) - 1);
        await appwrite.updateDocument(
            APPWRITE_USERS_COLLECTION_ID,
            receiverUserId,
        ...
        console.warn(`Receiver user document not found for ID: ${receiverUserId} during chat removal.`);
    }
...
const getChatState = async (currentUserId, connectionId) => {
    ...
    if (!connectionDoc || connectionDoc.status !== 'chat_active' ||
        (connectionDoc.senderId.$id !== currentUserId && connectionDoc.receiverId.$id !== currentUserId)) {
        const error = new Error("Unauthorized or invalid chat connection for state lookup");
        error.code = 403;
        throw error;
    }

    const partnerId = (connectionDoc.senderId.$id === currentUserId) ? connectionDoc.receiverId.$id : connectionDoc.senderId.$id;

    let partnerUser = null;
    try {
        partnerUser = await appwrite.getDocument(APPWRITE_USERS_COLLECTION_ID, partnerId);
    } catch (err) {
        console.warn(`Partner user document not found for ID: ${partnerId} during chat state lookup.`);
    }
    ...
    try {
        ...
        console.warn(`No images found for partnerId ${partnerId} during chat state lookup.`);
    }
    ...
    return {
        ...
        dateProposalDate: connectionDoc.dateProposalStatus && ['proposed', 'modified', 'accepted'].includes(connectionDoc.dateProposalStatus) ? connectionDoc.dateProposalDate : null,
        dateProposalPlace: connectionDoc.dateProposalStatus && ['proposed', 'modified', 'accepted'].includes(connectionDoc.dateProposalStatus) ? connectionDoc.dateProposalPlace : null,
        dateProposalProposerId: connectionDoc.dateProposalProposerId ? connectionDoc.dateProposalProposerId.$id : null,
        dateProposalLastActionBy: connectionDoc.dateProposalLastActionBy ? connectionDoc.dateProposalLastActionBy.$id : null,
        partnerId: partnerId,
        partnerName: partnerUser?.name || 'Unknown',
        partnerPhotoUrl: partnerPrimaryImage,
    };
 ...
 */
const sendMessage = async (currentUserId, connectionId, messageContent, messageType) => {
    const appwrite = new AppwriteService();
    ...
    if (!connectionDoc || connectionDoc.status !== 'chat_active' ||
        (connectionDoc.senderId.$id !== currentUserId && connectionDoc.receiverId.$id !== currentUserId)) {
        const error = new Error("Unauthorized or invalid chat connection to send message");
        error.code = 403;
        throw error;
    }
    ...
    if (messageType === 'text') {
        ...
        const error = new Error("Invalid messageType for sendMessage. Must be 'text' or 'image'.");
        error.code = 400;
        throw error;
    }
...
const proposeDate = async (currentUserId, connectionId, proposalDetails) => {
    ...
    if (!connectionDoc || connectionDoc.status !== 'chat_active' ||
        (connectionDoc.senderId.$id !== currentUserId && connectionDoc.receiverId.$id !== currentUserId)) {
        const error = new Error("Unauthorized or invalid chat connection for date proposal");
        error.code = 403;
        throw error;
    }
    ...
    if (['proposed', 'modified'].includes(connectionDoc.dateProposalStatus)) {
        const error = new Error("There is an active date proposal already! Please respond to it or wait for a response.");
...
const respondToDateProposal = async (currentUserId, connectionId, responseType, newDetails = {}) => {
    ...
    if (!connectionDoc || connectionDoc.status !== 'chat_active' ||
        (connectionDoc.senderId.$id !== currentUserId && connectionDoc.receiverId.$id !== currentUserId)) {
        const error = new Error("Unauthorized or invalid chat connection for date response");
        error.code = 403;
        throw error;
    }
    ...
    if (connectionDoc.dateProposalLastActionBy && connectionDoc.dateProposalLastActionBy.$id === currentUserId) {
    ...
    const currentMessageCount = connectionDoc.messageCount || 0;
    if (currentMessageCount >= MESSAGE_LIMIT) {
        const error = new Error("Message limit reached. Cannot respond to date proposal.");
        error.code = 403;
        throw error;
    }
    ...
    switch (responseType) {
        case 'accept':
            ...
            console.log(`Push Notification: Date confirmed between ${connectionDoc.senderId.$id} and ${connectionDoc.receiverId.$id}!`);
            break;
        case 'reject':
            updateData = {
        ...
        case 'modify':
            if (!newDetails.date || !newDetails.place) {
                const error = new Error("New date and place are required for modifying a proposal.");
                error.code = 400;
                throw error;
            }
            updateData = {
